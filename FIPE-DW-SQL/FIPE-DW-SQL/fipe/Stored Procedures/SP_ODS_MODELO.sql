CREATE PROCEDURE fipe.SP_ODS_MODELO
AS
BEGIN
	DECLARE @DTA_EXECUCAO DATETIME2(2) = GETDATE()
	MERGE INTO fipe.ODS_MODELO WITH (TABLOCK) AS DESTINO
		USING (											
			SELECT 
				 COD_MODELO
				,NME_MODELO
	...
	FROM fipe.ODS_MODELO M 
	INNER JOIN (
				SELECT DISTINCT COD_MODELO, COD_FIPE
					FROM fipe.STG_DADOS_MODELO
				) DM
