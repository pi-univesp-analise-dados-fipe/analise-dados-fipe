CREATE   PROCEDURE [fipe_dw].[SP_DW_FTO_DADOS_MODELO]
	@DTA_REFERENCIA INT = NULL,
	@FLG_REPROCESSAMENTO BIT 
AS
BEGIN
	DECLARE @DTA_EXECUCAO DATETIME2(2) = GETDATE()
	IF @DTA_REFERENCIA IS NULL AND @FLG_REPROCESSAMENTO = 0 
		SET @DTA_REFERENCIA = (SELECT LEFT(FORMAT(DATEADD(MM, 1, CAST(CONCAT(MAX(COD_ANO_MES), '01') AS DATE)), 'yyyyMMdd'), 6) 
	...
	*/
	...
	MERGE INTO fipe_dw.DW_FTO_DADOS_MODELO  WITH (TABLOCK) AS DESTINO
		USING (											
					SELECT 
					 TM.COD_ANO_MES AS DTA_REFERENCIA
			...
			WHERE	
				...
				TM.COD_ANO_MES = @DTA_REFERENCIA OR @DTA_REFERENCIA  IS NULL
		) AS ORIGEM
	ON 
		(
			DESTINO.[DTA_REFERENCIA] 		= ORIGEM.[DTA_REFERENCIA] 	AND 
