...
def save_todos_modelos_por_marca(tipoVeiculo, qtdAnosRetroativos, codigoMesReferencia, verbose):
    ...
    for marca in marcas:
        ...
        if (len(listaDadosModelo) > 0):
            save_file_json(listaDadosModelo,
                           f"modelos/{tipoVeiculo.name}/dados_modelo_{tipoVeiculo.name}_mes_{codigoMesReferencia}_marca_{marca['nomeMarca']}",
                           False)
            print(
                f"Salvo os modelos da marca {marca['nomeMarca']}. Tipo de veiculo: {tipoVeiculo.name}. Mes Referencia: {codigoMesReferencia}. Data: {datetime.datetime.now()}")
...
def save_modelos_por_marca(tipoVeiculo, qtdAnosRetroativos, codigoMesReferencia, codigoMarca):
    ...
    if (len(listaDadosModelo) > 0):
        save_file_json(listaDadosModelo,
                       f"modelos/{tipoVeiculo.name}/dados_modelo_{tipoVeiculo.name}_mes_{codigoMesReferencia}_marca_{codigoMarca}",
                       False)
        print(
            f"Salvo os modelos da marca {codigoMarca}. Tipo de veiculo: {tipoVeiculo.name}. Mes Referencia: {codigoMesReferencia}. Data: {datetime.datetime.now()}")
...
def get_dados_modelo(codigoTipoVeiculo, codigoMesReferencia, codigoMarca, codigoModelo, anoModelo,
                     codigoTipoCombustivel):
...
def get_amostra_dados_todos_modelos(tamanhoAmostra, tipoVeiculo, verbose):
    ...
    for modelo in modelos:
        if (verbose and (i % 100 == 0)):
            print(
                f"Lendo dados de amostra do modelo {i + 1} de {tamanhoAmostra}. Tipo de veiculo: {tipoVeiculo.name}. Modelo: {modelo}")
        codigoTipoVeiculo = int(modelo["codigoTipoVeiculo"])
...
def save_json_dados_todos_modelos(dadosModelo, tipoVeiculo, flgAmostra, codigoMesReferencia):
    if (flgAmostra):
        save_file_json(dadosModelo,
                       f"modelos/{tipoVeiculo.name}/amostra_dados_modelo_{tipoVeiculo.name}_{len(dadosModelo)}_{codigoMesReferencia}",
                       False)
    else:
        save_file_json(dadosModelo, f"modelos/{tipoVeiculo.name}/dados_modelo_{tipoVeiculo.name}_{codigoMesReferencia}",
                       False)
...
def save_json_dados_todos_modelos_periodo(codigoMesReferencia, tipoVeiculo, qtdAnosRetroativos, verbose):
    ...
    save_json_dados_todos_modelos(dadosModelo=dados, tipoVeiculo=tipoVeiculo, flgAmostra=False,
                                  codigoMesReferencia=codigoMesReferencia)
    print(
        f"Salvo os dados dos modelos do periodo {codigoMesReferencia}. Tipo de Veiculo: {tipoVeiculo.name}. Data: {datetime.datetime.now()}")
...
if __name__ == '__main__':
    ...
    # print(f"Salva a lista de modelos: {datetime.now()}")
    mes_referencia = 251
    marcas_pendentes = [58, 59, 120, 123, 125, 127, 136, 140, 147, 149, 152, 153, 154, 156, 157, 161,
                        ]

    # for ano in anos_pendentes:
    # save_todos_modelos_por_marca(TipoVeiculoFipe.carro, 15, 295, False)
    for marca in marcas_pendentes:
        save_modelos_por_marca(tipoVeiculo=TipoVeiculoFipe.carro, codigoMesReferencia=mes_referencia, codigoMarca=marca,
                               qtdAnosRetroativos=15)
