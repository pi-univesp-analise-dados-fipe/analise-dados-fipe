...
def save_json_todas_marcas(codigoMesReferencia):
    if(codigoMesReferencia is None):
        codigoMesReferencia = getMaxReferencia()
    listaMarcas = list()
    for tipo in TipoVeiculoFipe:
        marcas = get_marcas_por_tipo(tipo.value, codigoMesReferencia)
        for marca in marcas:
            listaMarcas.append(marca)
    #    print(marcas)
    #save_file_json(listaMarcas, f"marcas/marcas_{tipo.name}", False)
    #print(listaMarcas)
    save_file_json(listaMarcas, f"marcas/marcas_{codigoMesReferencia}", False)
...
def save_json_todos_modelos(listaModelos, codigoMesReferencia, tipoVeiculo):
    save_file_json(listaModelos, f"modelos/modelos_{tipoVeiculo.name}_{codigoMesReferencia}", False)
...
if __name__ == '__main__':
    ...
    """
    periodos_referencia = get_periodo_referencia()
    for periodo in periodos_referencia:
        codigoMesReferencia = periodo["Codigo"]
        #save_json_todas_marcas(codigoMesReferencia=codigoMesReferencia)
        if (codigoMesReferencia < 34):
            save_json_todos_modelos(get_todos_modelos(TipoVeiculoFipe.moto, codigoMesReferencia), codigoMesReferencia, TipoVeiculoFipe.moto)
            save_json_todos_modelos(get_todos_modelos(TipoVeiculoFipe.caminhao, codigoMesReferencia), codigoMesReferencia, TipoVeiculoFipe.caminhao)
